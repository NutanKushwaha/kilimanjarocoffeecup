...
class Flexi_auth_lite
{
	...
	/**
	 * get_all_privilegs
	 * Return all privilegs as per database .
	 *
	 * @return bool
	 * @author Rob Hussey
	 */
	public function get_all_privilegs()
	{
	...
	public function is_privileged($privileges = FALSE)
	{
		// Get users privileges and convert names to lowercase for comparison.
		$user_privileges = array();
		if (! empty($this->CI->auth->session_data[$this->CI->auth->session_name['user_privileges']]))
		{
			foreach($this->CI->auth->session_data[$this->CI->auth->session_name['user_privileges']] as $id => $name)
			{
				$user_privileges[$id] = strtolower($name);
			}
		}
	...
	/**
	 * get_comp_admin_company_code
	 * Get the Company admin code.
	 *
	 * @return void
	 * @author Vishal
	 */
	public function get_comp_admin_company_code()
	{
        $param = [];
        $param['from_tbl'] = 'company';
        $param['select'] = 'company_code';
        $param['result'] = 'row';
        $param['where']['account_id'] = $this->get_user_id();
        return $this->CI->Commonmodel->get_all($param)['company_code'];		
	}

	/**
	 * get_user_custom_data
	 * Get the users user custom data from the session related to pass in it.
	 *
	 * @return void
	 * @author Rob Hussey
	 */
	public function get_user_custom_data( $user_custom_key = null )
	{		
            if( is_null($user_custom_key) OR empty($user_custom_key)) {
                return $this->CI->auth->session_data[$this->CI->auth->session_name['custom_user_data']];
            }
            return ($this->CI->auth->session_data[$this->CI->auth->session_name['custom_user_data']][$user_custom_key] !== FALSE) ? 
                    $this->CI->auth->session_data[$this->CI->auth->session_name['custom_user_data']][$user_custom_key] : FALSE;
	}
